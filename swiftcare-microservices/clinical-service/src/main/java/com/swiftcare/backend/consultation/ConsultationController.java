...
public class ConsultationController {
    ...
    public ResponseEntity<List<DoctorResponse>> getAvailableDoctors(
        ...
        ensureActivePremiumPatient(authentication);
    ...
    public ResponseEntity<ConsultationResponse> bookConsultation(
        ...
        UUID patientId = ensureActivePremiumPatient(authentication);
    ...
    private void ensureCanAccess(
        ...
        if (hasRole(authentication, "PATIENT")) {
            if (!ensureActivePremiumPatient(authentication).equals(consultation.getPatientId())) {
                throw new SecurityException(
    ...
    private UUID ensureActivePremiumPatient(Authentication authentication) {
        Patient patient = getPatient(authentication);

        if (patient.getTier() != Tier.PREMIUM
                || !patientRepository.hasActivePremiumSubscription(patient.getId())) {
    ...
    private UUID getPatientId(Authentication authentication) {
        return getPatient(authentication).getId();
    }
